...
<body class="bg-gray-50 min-h-screen flex flex-col">
            ...
            <div class="bg-white rounded-xl border p-6">
                <div class="flex items-center justify-between">
                    ...
                    <div class="h-12 w-12 bg-yellow-100 rounded-lg flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-yellow-700" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2">
            ...
            <div id="openPracticesModal"
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="h-12 w-12 bg-orange-100 rounded-lg flex items-center justify-center">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-orange-600" viewBox="0 0 24 24"
                            fill="none" stroke="currentColor" stroke-width="2">
                            <circle cx="12" cy="12" r="10" />
        ...
        <div class="min-h-full flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
                <div class="flex items-start justify-between px-6 py-5 border-b">
                    <h3 class="text-2xl font-semibold leading-6 text-red-600">Remove Team</h3><button
                        id="closeRemoveTeamModal" class="text-gray-500 text-2xl">&times;</button>
                </div>
                <div class="px-6 py-6">
                    <p class="text-gray-600">Are you sure you want to remove <strong class="text-gray-900">{{ team.name }}</strong>? <br><span class="text-sm text-red-600 mt-1 block">Warning: This action
                            cannot
                            be
                            undone.</span></p>
                    <div class="mt-6 flex justify-end gap-3"><button id="cancelRemoveTeamModal"
                            class="px-4 h-11 border rounded-xl">Cancel</button>
                        <form method="post" action="{% url 'delete_team' team.id %}">{% csrf_token %}<button
                                type="submit" class="px-5 h-11 bg-red-600 text-white rounded-xl">Yes, Remove
                                Team</button></form>
                    </div>
                </div>
            </div>
    ...
    <div id="addPlayerModal" class="hidden fixed inset-0 bg-black/60 z-50">
        <div class="min-h-full flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl">
                ...
                <form method="post" action="{% url 'add_player' team.id %}" class="px-6 py-6 space-y-6">{% csrf_token %}
                    <div class="grid grid-cols-2 gap-4"><label><span class="text-sm font-medium">First Name</span><input
                                name="first_name" type="text" class="mt-2 block w-full h-12 px-4 border rounded-xl"
                                required></label><label><span class="text-sm font-medium">Last Name</span><input
                                name="last_name" type="text" class="mt-2 block w-full h-12 px-4 border rounded-xl"
                                required></label></div><label><span class="text-sm font-medium">Email</span><input
                            name="email" type="email" class="mt-2 block w-full h-12 px-4 border rounded-xl"></label>
                    <div class="grid grid-cols-2 gap-4"><label><span class="text-sm font-medium">Jersey</span><input
                                ...
                                class="text-sm font-medium">Position</span><select name="position"
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md">
                ...
                <div class="px-6 py-6">
                    ...
                    <div class="mt-6 flex justify-end">
                        <button id="okMaxCapacityModal"
                            class="px-5 h-11 bg-gray-900 text-white rounded-xl hover:bg-black font-medium">OK</button>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl">
                ...
                <form method="post" id="editPlayerForm" class="px-6 py-6 space-y-6">{% csrf_token %}<div
                        class="grid grid-cols-2 gap-4"><label><span class="text-sm font-medium">First Name</span><input
                                id="editFirstName" name="first_name" type="text"
                                class="mt-2 block w-full h-12 px-4 border rounded-xl" required></label><label><span
                                class="text-sm font-medium">Last Name</span><input id="editLastName" name="last_name"
                                type="text" class="mt-2 block w-full h-12 px-4 border rounded-xl" required></label>
                    </div><label><span class="text-sm font-medium">Email</span><input id="editEmail" name="email"
                            type="email" class="mt-2 block w-full h-12 px-4 border rounded-xl"></label>
                    <div class="grid grid-cols-2 gap-4"><label><span class="text-sm font-medium">Jersey</span><input
    ...
    <div id="removePlayerModal" class="hidden fixed inset-0 bg-black/60 z-50">
        <div class="min-h-full flex items-center justify-center p-4">
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
                ...
                <div class="px-6 py-6">
                    <p class="text-sm text-gray-600">Are you sure you want to remove <strong
                            id="removePlayerName"></strong>?</p>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Custom logic for Add Player Modal - Max Capacity Check
            setupModal('addPlayerModal', null, ['closeAddPlayerModal', 'cancelAddPlayerModal']);
            setupModal('maxCapacityModal', null, ['closeMaxCapacityModal', 'okMaxCapacityModal']);

            const addPlayerBtn = document.getElementById('openAddPlayerModal');
            ...
            // Using default:0 filter to ensure we get a number even if None/Empty
            const currentCount = {{ players.count }};
            const maxCapacity = {{ team.max_players_allowed|default:0 }};

            if (addPlayerBtn) {
                addPlayerBtn.addEventListener('click', (e) => {
                    e.preventDefault();
                    // Check if maxCapacity is set (greater than 0) and if we reached it
                    if (maxCapacity > 0 && currentCount >= maxCapacity) {
                        ...
                        if (addPlayerModal) addPlayerModal.classList.remove('hidden');
                    }
                });
            }
